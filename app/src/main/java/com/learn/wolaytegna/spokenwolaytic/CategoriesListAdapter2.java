...
public class CategoriesListAdapter2 extends ArrayAdapter<Categories1> implements TextToSpeech.OnInitListener {

    private int lastPosition = -1;
    private Context context;
    private int mResource;
    private ArrayList<Categories1> arrayList;
   ...
   public CategoriesListAdapter2(Context context, int resource, ArrayList<Categories1> objects) {
    ...
    @Override
    public Categories1 getItem(int position) {
    ...
    public View getView(int position, View convertView, @NonNull final ViewGroup parent) {
       ...
       Categories1 categories = new Categories1(getItem(position).getEnglish(),getItem(position).getWolaytegna(),getItem(position).getImage_id());
       ...
       final ViewHolder holder;
        ...
        holder.image_id = categories.getImage_id();

        SQLiteDatabase db1;
        DBOpenHelper helper1 = new DBOpenHelper(context);

        db1 = helper1.getWritableDatabase();

        Cursor c1 = db1.rawQuery("SELECT * FROM " + DBOpenHelper.TABLE_WOLAYTA + ";", null);

        while (c1.moveToNext()) {
            if (c1.getString(1).equals(holder.englishView.getText().toString())){
